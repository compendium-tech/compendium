...
const fetchUserData = async () => {
  ...
  try {
    ...
    user.value = data;
    editableName.value = data.name || ''; // Initialize with name or empty string
  } catch (err: any) {
    accountError.value = err.message || 'Failed to fetch user data.';
    editableName.value = ''; // Default to empty if fetch fails
  } finally {
...
const toggleEditName = async () => {
  ...
  if (isEditingName.value) {
    // We are currently in editing mode, so this click means "Save"
    ...
    try {
      ...
      isEditingName.value = false; // Turn off editing mode
    } catch (err: any) {
    ...
    // We are not in editing mode, so this click means "Edit"
    editableName.value = user.value.name || ''; // Ensure it's set even if name is null
    isEditingName.value = true; // Turn on editing mode
  }
};
      ...
      <div v-if="subscriptionResponse && subscriptionResponse.subscription && !isLoadingSubscription">
        <div class="bg-gray-50 rounded-lg p-6 shadow-sm">
          ...
          <div class="mt-6 text-right" v-if="isPayer">
            <BaseButton @click="handleCancelSubscription" variant="secondary" :is-loading="isCancelling" size="sm">
              Cancel Subscription
            </BaseButton>
          </div>
