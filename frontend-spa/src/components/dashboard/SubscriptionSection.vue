    ...
    <div class="bg-gray-50 rounded-lg p-6 shadow-sm">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p class="mt-1 text-lg text-gray-900">{{ dateToString(subscriptionResponse.subscription.since) }}
          </p>
        </div>
      ...
      <div class="mt-6 text-right" v-if="isPayer">
        <BaseButton @click="handleCancelSubscription" variant="secondary" :is-loading="isLoading.cancellingSubscription"
          size="sm">
          Cancel Subscription
        </BaseButton>
      </div>
        ...
        <div class="mb-6">
          ...
          <div v-else-if="invitationCode">
            <div class="flex items-center space-x-4 mb-4">
              ...
              <BaseButton variant="outline" class="flex" @click="updateInvitationCode"
                :is-loading="isLoading.updatingInvitationCode" hover-effect="scale">
                <Icon icon="mdi:refresh" class="h-5 w-5 mr-2" />
                <span>Update</span>
              </BaseButton>
              <BaseButton variant="secondary" class="flex" @click="removeInvitationCode"
                :is-loading="isLoading.removingInvitationCode">
          ...
          <div v-else>
            ...
            <BaseButton @click="updateInvitationCode" class="flex" :is-loading="isLoading.updatingInvitationCode"
              size="sm">
              <Icon icon="mdi:refresh" class="h-5 w-5 mr-2" />
              Generate
            </BaseButton>
          ...
          <ul class="space-y-4">
            <li v-for="member in subscriptionResponse.subscription.members" :key="member.userId"
              ...
              <div class="flex items-center justify-between space-x-2">
                <p class="font-semibold text-gray-900">{{ member.name }} <span v-if="member.email">&lt;{{ member.email
                    }}&gt;</span></p>
  ...
  <div v-if="!subscriptionResponse?.isActive && !isLoading.subscription">
    ...
    <div class="mt-8 bg-white rounded-lg p-6 shadow-sm border border-primary-200">
      ...
      <div class="flex items-center space-x-2">
        ...
        <BaseButton @click="handleJoinSubscription" class="flex" :is-loading="isLoading.joiningSubscription" size="sm">
